...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run BirdCLEF teacher-ensemble Kaggle submission with hidden-test/dry-run fallback."
    )
    parser.add_argument(
        "--artifact_dir",
        required=True,
        help="Mounted Kaggle dataset containing teacher artifacts.",
    )
    parser.add_argument(
        "--competition_dir",
    ...
    parser.add_argument(
        ...
        help="Writable output directory for submission and dry-run outputs.",
    )
    parser.add_argument(
        "--dryrun_n_files",
        type=int,
...
def main() -> None:
    ...
    if hidden_test_files:
        run_infer(
            [
                sys.executable,
                str(infer_script),
                "--teacher_manifest_json",
                str(teacher_manifest_json),
                "--audio_dir",
                str(test_dir),
    ...
    print("Kaggle's hidden rerun should overwrite submission.csv with real teacher predictions.")
